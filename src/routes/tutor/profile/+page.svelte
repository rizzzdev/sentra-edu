...
import { userStore, subjectStore, educationLevelStore, enrollmentStore, jobStore, attendanceStore, payrollStore } from '$lib/api';
  ...
  const studentPrograms = $derived(
    currentUser
      ? $enrollmentStore.filter((enrollmentItem) => enrollmentItem.deletedAt === null && enrollmentItem.studentId === currentUser?.id).length
      : 0
  );
      ...
      <div class="stat-grid mb-0">
        {#if currentUser.role === 'SUPER_ADMIN'}
          <div class="stat">
            ...
            <div>
              <div class="s-val">{$jobStore.length}</div>
          ...
          <div class="stat">
            ...
            <div>
              <div class="s-val">{$enrollmentStore.length}</div>
          ...
          <div class="stat">
            ...
            <div>
              <div class="s-val">{$userStore.filter((userItem) => userItem.role === 'TENTOR').length}</div>
        ...
        {:else if currentUser.role === 'STUDENT'}
          ...
          <div class="stat">
            ...
            <div>
              <div class="s-val">{$attendanceStore.filter((attendanceItem) => attendanceItem.status === 'APPROVED').length}</div>
              <div class="s-lbl">Sesi Disetujui</div>
            </div>
          </div>
        {:else if currentUser.role === 'PARENT'}
          <div class="stat">
            ...
            <div>
              <div class="s-val">{parentChildren}</div>
          ...
          <div class="stat">
            ...
            <div>
              <div class="s-val">{$enrollmentStore.filter((enrollmentItem) => enrollmentItem.parentId === currentUser?.id).length}</div>
