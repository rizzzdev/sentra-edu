  ...
  const myActiveLessons = $derived.by(() => {
    ...
    for (const job of assignedJobs) {
      ...
      const studentNames = Array.isArray(job.studentNames) && job.studentNames.length > 0
        ...
        : (job.studentName || (job.studentId ? $userStore.find((userItem) => userItem.id === job.studentId)?.fullName : 'Murid'));

      const subjectIds = Array.isArray(job.subjectIds) && job.subjectIds.length > 0 ? job.subjectIds : (job.subjectId ? [job.subjectId] : []);
      const subjectNames = $subjectStore.filter((subjectItem) => subjectIds.includes(subjectItem.id)).map((subjectItem) => subjectItem.name).join(', ') || 'Mapel';

      const classIds = Array.isArray(job.classIds) && job.classIds.length > 0 ? job.classIds : (job.classId ? [job.classId] : []);
      const classNames = $classStore.filter((classItem) => classIds.includes(classItem.id)).map((classItem) => classItem.className).join(', ') || 'Kelas';
          ...
          <table class="tbl">
            ...
            <tbody>
              {#each myApps as app (app.id)}
                {@const job = $jobStore.find((jobItem) => jobItem.id === app.jobId)}
                <tr>
