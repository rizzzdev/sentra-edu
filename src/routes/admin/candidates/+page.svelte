...
import { subjectStore, educationLevelStore, candidateStore } from '$lib/api';
  ...
  async function handleScheduleTestSubmit() {
    ...
    const response = await api.candidates.create({
      ...scheduleTestCand,
  ...
  async function handleRecordTestSubmit() {
  ...
  async function handleRecordInterviewSubmit() {
  ...
  async function handleAcceptSubmit() {
    ...
    const response = await api.users.create({
      fullName: acceptCand.fullName,
      email: acceptCand.email,
      role: 'TENTOR',
      password: initialPassword || 'tentor123'
    });
    if (!response.error) {
  ...
  async function handleConfirmDelete() {
    ...
    const response = await api.candidates.delete(deletingCandidateId);
