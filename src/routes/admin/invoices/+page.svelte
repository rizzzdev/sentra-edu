...
import { userStore, packageStore, enrollmentStore, invoiceStore } from '$lib/api';
  ...
  function getPackageName(enrollmentId: string): string {
    const enrollmentItem = $enrollmentStore.find((enrollment) => enrollment.id === enrollmentId);
    if (!enrollmentItem) return '—';
    return $packageStore.find((packageItem) => packageItem.id === enrollmentItem.packageId)?.name || '—';
  }
  ...
  async function handleConfirmDelete() {
    ...
    const response = await api.invoices.delete(deletingInvoiceId);
...
<div class="filter-bar">
  ...
  <SelectSearch
    ...
    options={[
      ...
      ...Object.entries(INVOICE_STATUS_LABEL).map(([statusKey, statusLabel]) => ({ value: statusKey, label: statusLabel }))
    ]}
    className="max-w-48"
  />
</div>
