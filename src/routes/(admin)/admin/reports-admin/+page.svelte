<script lang="ts">
  ...
  $: paidInvs = $dbStore.invoices.filter((invoiceItem) => invoiceItem.deletedAt === null && invoiceItem.status === 'PAID');
  $: unpaidInvs = $dbStore.invoices.filter((invoiceItem) => invoiceItem.deletedAt === null && invoiceItem.status === 'UNPAID');
  ...
  $: reqClaims = $dbStore.payrollClaims.filter((claimItem) => claimItem.deletedAt === null && claimItem.status === 'REQUESTED');
  ...
  $: levelPrograms = (() => {
    ...
    $dbStore.enrollments
      ...
      .forEach((enrollmentItem) => {
        const classItem = $dbStore.classes.find((cls) => cls.id === enrollmentItem.classId);
        const levelItem = classItem ? $dbStore.educationLevels.find((lvl) => lvl.id === classItem.educationLevelId) : null;
  ...
  function getUserName(userId: string): string {
    return $dbStore.users.find((userItem) => userItem.id === userId)?.fullName || 'Tentor';
  }
</script>
