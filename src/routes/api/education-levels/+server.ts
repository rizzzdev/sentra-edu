...
import { educationLevelService, requireAdmin, isValidId, sanitizeInput } from '$lib/server';
...
export const PUT: RequestHandler = async ({ request, cookies }) => {
  ...
  const result = await educationLevelService.update(id, { ...body, updatedAt: now });
  return json(result);
};
