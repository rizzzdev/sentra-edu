...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (body.id) {
      ...
      await sql`INSERT INTO jobs (id,title,class_id,subject_id,package_id,job_mode,tentor_fee,session_duration_minutes,schedule_days,schedule_time,student_count,location,latitude,longitude,status,assigned_tentor_id,student_id,enrollment_id,notes,created_at,updated_at) VALUES (${id},${body.title},${body.classId},${body.subjectId},${body.packageId},${body.jobMode??body.mode??'OFFLINE'},${body.tentorFee??120000},${body.sessionDurationMinutes??90},${body.scheduleDays??['Senin']},${body.scheduleTime??'16:00'},${body.studentCount??1},${body.location??'Lokasi Siswa'},${body.latitude},${body.longitude},'AVAILABLE',${body.assignedTentorId},${body.studentId},${body.enrollmentId},${body.notes??''},${now},${now})`;
