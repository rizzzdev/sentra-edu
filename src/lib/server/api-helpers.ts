...
function mapAttendanceRow(row: DatabaseRow) {
  return {
    ...
    tentorId: String(row.tentor_id),
    subjectIds: Array.isArray(row.subject_ids) ? (row.subject_ids as string[]).map(String) : [],
    classIds: Array.isArray(row.class_ids) ? (row.class_ids as string[]).map(String) : [],
    studentIds: Array.isArray(row.student_ids) ? (row.student_ids as string[]).map(String) : [],
    studentNames: Array.isArray(row.student_names) ? (row.student_names as string[]).map(String) : [],
    sessionDate: String(row.session_date),
    startTime: String(row.start_time || ''),
    endTime: String(row.end_time || ''),
    durationMinutes: row.duration_minutes !== null && row.duration_minutes !== undefined ? Number(row.duration_minutes) : 90,
    sessionsCount: row.sessions_count !== null && row.sessions_count !== undefined ? Number(row.sessions_count) : 1,
